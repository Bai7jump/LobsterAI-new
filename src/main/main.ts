...
import { getOpenClawInstancePool } from './libs/openClawInstancePool';
import {
  listPairingRequests,
  approvePairingCode,
...
import { ScheduledTaskInstanceRouter } from './libs/scheduledTaskInstanceRouter';
...
import { registerOpenClawTeamIPC } from './ipcOpenClawTeam';
...
>>>>>>> 02a8f761607a9fd976c9446cf674c48b34ee99af
  }
  return coworkRunner;
};
...
const getCronJobService = (): CronJobService => {
  if (!cronJobService) {
    ...
    const instancePool = getOpenClawInstancePool();

    // 使用真实的实例路由器，数据源为 OpenClawTeamManager
    const instanceRouter = new ScheduledTaskInstanceRouter();

    cronJobService = new CronJobService({
      getGatewayClient: (instanceId?: string) => {
        // 如果没有指定实例ID，使用默认适配器
        if (!instanceId) {
          return adapter.getGatewayClient();
        }

        // 根据实例ID获取对应的实例管理器
        const poolInstance = instancePool.getInstance(instanceId);
        if (poolInstance) {
          // 从实例的manager中获取网关客户端
          // TODO: 实现多实例网关客户端获取逻辑，需要OpenClawEngineManager支持
          return adapter.getGatewayClient();
        }

        return adapter.getGatewayClient();
      },
      ensureGatewayReady: async (instanceId?: string) => {
        // 如果没有指定实例ID，使用默认适配器
        if (!instanceId) {
        ...
        // 根据实例ID启动对应实例
        const poolInstance = instancePool.getInstance(instanceId);
        if (poolInstance && poolInstance.status.phase !== 'running') {
          await instancePool.startInstance(instanceId);
        }
...
if (!gotTheLock) {
  ...
  // IPC 处理程序
  registerOpenClawTeamIPC();

  ipcMain.handle('store:get', (_event, key: unknown) => {
    // 安全验证：确保 key 是字符串
    if (typeof key !== 'string') {
      console.warn('[Security] Invalid store:get call: key must be a string');
      return undefined;
    }
    const trimmedKey = key.trim();
    if (!trimmedKey) {
      console.warn('[Security] Invalid store:get call: key cannot be empty');
      return undefined;
    }
    return getStore().get(trimmedKey);
  });

  ipcMain.handle('store:set', async (_event, key: unknown, value: unknown) => {
    // 安全验证：确保 key 是字符串
    if (typeof key !== 'string') {
      console.warn('[Security] Invalid store:set call: key must be a string');
      return { success: false, error: 'Invalid parameter: key must be a string' };
    }
    const trimmedKey = key.trim();
    if (!trimmedKey) {
      console.warn('[Security] Invalid store:set call: key cannot be empty');
      return { success: false, error: 'Invalid parameter: key cannot be empty' };
    }
    getStore().set(trimmedKey, value);
    if (trimmedKey === 'app_config') {
      const syncResult = await syncOpenClawConfig({
  ...
  ipcMain.handle('store:remove', (_event, key: unknown) => {
    // 安全验证：确保 key 是字符串
    if (typeof key !== 'string') {
      console.warn('[Security] Invalid store:remove call: key must be a string');
      return { success: false, error: 'Invalid parameter: key must be a string' };
    }
    const trimmedKey = key.trim();
    if (!trimmedKey) {
      console.warn('[Security] Invalid store:remove call: key cannot be empty');
      return { success: false, error: 'Invalid parameter: key cannot be empty' };
    }
    getStore().delete(trimmedKey);
    return { success: true };
  });
  ...
  ipcMain.on('network:status-change', (_event, status: unknown) => {
    // 安全验证：确保 status 是合法的值
    if (typeof status !== 'string' || (status !== 'online' && status !== 'offline')) {
      console.warn('[Security] Invalid network:status-change call:', status);
      return;
    }
  ...
  ipcMain.handle('shell:openPath', async (_event, filePath: unknown) => {
    try {
      ...
      if (typeof filePath !== 'string') {
        return { success: false, error: 'Invalid parameter: filePath must be a string' };
      }
      const trimmedPath = filePath.trim();
      if (!trimmedPath) {
        return { success: false, error: 'Invalid parameter: filePath cannot be empty' };
      }
      // 安全验证：防止路径遍历攻击
      if (trimmedPath.includes('../') || trimmedPath.includes('..\\')) {
        return { success: false, error: 'Invalid parameter: path traversal not allowed' };
      }
      const normalizedPath = normalizeWindowsShellPath(trimmedPath);
  ...
  ipcMain.handle('shell:openExternal', async (_event, url: unknown) => {
    try {
      ...
      if (typeof url !== 'string') {
        return { success: false, error: 'Invalid parameter: url must be a string' };
      }
      const trimmedUrl = url.trim();
      if (!trimmedUrl) {
        return { success: false, error: 'Invalid parameter: url cannot be empty' };
      }
      // 安全验证：只允许安全的协议
      ...
      try {
        ...
        if (!allowedProtocols.has(parsedUrl.protocol)) {
          return { success: false, error: 'Invalid parameter: URL protocol not allowed' };
        }
      } catch {
        // 如果 URL 解析失败，拒绝访问
        return { success: false, error: 'Invalid parameter: invalid URL format' };
      }
