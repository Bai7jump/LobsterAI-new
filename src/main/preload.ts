...
contextBridge.exposeInMainWorld('electron', {
  ...
  openClawTeam: {
    // 实例管理
    listInstances: () => ipcRenderer.invoke('openClawTeam:listInstances'),
    startInstance: (request: { instanceId: string }) => ipcRenderer.invoke('openClawTeam:startInstance', request),
    stopInstance: (request: { instanceId: string }) => ipcRenderer.invoke('openClawTeam:stopInstance', request),
    createInstance: (request: {
      name: string;
      type?: 'general' | 'specialized';
      capabilities?: string[];
      config?: any;
    }) => ipcRenderer.invoke('openClawTeam:createInstance', request),
    deleteInstance: (request: { instanceId: string }) => ipcRenderer.invoke('openClawTeam:deleteInstance', request),

    // 任务管理
    submitTask: (request: { input: any; scheduledTaskId?: string }) =>
      ipcRenderer.invoke('openClawTeam:submitTask', request),
    getTaskStatus: (request: { taskId: string }) => ipcRenderer.invoke('openClawTeam:getTaskStatus', request),
    listTasks: (request?: {
      filter?: {
        status?: string;
        assignedTo?: string;
    ...
    }) => ipcRenderer.invoke('openClawTeam:listTasks', request),

    // 配置管理
    getConfig: () => ipcRenderer.invoke('openClawTeam:getConfig'),
    updateConfig: (request: { config: any }) => ipcRenderer.invoke('openClawTeam:updateConfig', request),

    // 事件订阅
    subscribe: () => ipcRenderer.send('openClawTeam:subscribe'),
    unsubscribe: () => ipcRenderer.send('openClawTeam:unsubscribe'),
    onEvent: (callback: (event: any) => void) => {
      const handler = (_event: any, data: any) => callback(data);
      ipcRenderer.on('openClawTeam:event', handler);
      return () => ipcRenderer.removeListener('openClawTeam:event', handler);
    },
  },
  cowork: {
    // Session management
