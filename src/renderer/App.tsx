...
import OpenClawTeamView from './components/openClawTeam/OpenClawTeamView';
...
const App: React.FC = () => {
  ...
  const [mainView, setMainView] = useState<'cowork' | 'skills' | 'scheduledTasks' | 'mcp' | 'openClawTeam'>('cowork');
...
>>>>>>> 02a8f761607a9fd976c9446cf674c48b34ee99af
  ...
  const handleShowOpenClawTeam = useCallback(() => {
    setMainView('openClawTeam');
  }, []);
  ...
  // 监听托盘菜单打开设置的事件
  useEffect(() => {
    const unsubscribe = window.electron.app.onOpenSettings(() => {
      handleShowSettings();
    });
    return unsubscribe;
  }, [handleShowSettings]);
  ...
  // 监听托盘菜单新建任务的事件
  useEffect(() => {
    const unsubscribe = window.electron.app.onNewTask(() => {
      handleNewChat();
    });
    return unsubscribe;
  }, [handleNewChat]);
    ...
    <div className="h-screen overflow-hidden flex flex-col dark:bg-claude-darkSurfaceMuted bg-claude-surfaceMuted">
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        <Sidebar
          ...
          onShowOpenClawTeam={handleShowOpenClawTeam}
