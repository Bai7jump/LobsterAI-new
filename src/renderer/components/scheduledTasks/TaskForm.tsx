...
interface FormState {
  ...
  deliveryTo: string;
  teamConfig: ScheduledTaskTeamConfig;
}
...
function createFormState(task?: ScheduledTask, template?: TaskFormProps['initialTemplate']): FormState {
  if (!task && !template) {
  ...
  if (task) {
    const nextState: FormState = {
      ...
      teamConfig: task.openclawTeam ? {
        assignmentStrategy: task.openclawTeam.assignmentStrategy || 'round-robin',
        instanceId: task.openclawTeam.instanceId,
        affinityTags: task.openclawTeam.affinityTags,
        capabilityRequirements: task.openclawTeam.capabilityRequirements,
        retryStrategy: task.openclawTeam.retryStrategy || {
          maxRetries: 3,
          retryDelayMs: 1000,
          failoverToOtherInstance: true,
        },
      } : DEFAULT_FORM_STATE.teamConfig,
    };
  ...
  // 从模板创建
  const nextState: FormState = { ...DEFAULT_FORM_STATE };

  if (template) {
    if (template.name) nextState.name = template.name;
    if (template.description) nextState.description = template.description;
    if (template.sessionTarget) nextState.sessionTarget = template.sessionTarget;
    if (template.wakeMode) nextState.wakeMode = template.wakeMode;

    if (template.payload) {
      if (template.payload.kind === 'systemEvent') {
        nextState.payloadKind = 'systemEvent';
        nextState.payloadText = template.payload.text || '';
      } else if (template.payload.kind === 'agentTurn') {
        nextState.payloadKind = 'agentTurn';
        nextState.payloadText = template.payload.message || '';
        if (typeof template.payload.timeoutSeconds === 'number') {
          nextState.timeoutSeconds = String(template.payload.timeoutSeconds);
        }
      }
    }

    if (template.delivery) {
      nextState.deliveryMode = template.delivery.mode || 'none';
      nextState.deliveryChannel = template.delivery.channel || 'last';
      nextState.deliveryTo = template.delivery.to || '';
    }

    if (template.schedule) {
      if (template.schedule.kind === 'at') {
        nextState.scheduleKind = 'at';
        nextState.scheduleAt = toDatetimeLocalValue(template.schedule.at);
      } else if (template.schedule.kind === 'every') {
        nextState.scheduleKind = 'every';
        const parsedEvery = parseEverySchedule(template.schedule.everyMs);
        nextState.everyAmount = parsedEvery.everyAmount;
        nextState.everyUnit = parsedEvery.everyUnit;
      } else if (template.schedule.kind === 'cron') {
        nextState.scheduleKind = 'cron';
        nextState.cronExpr = template.schedule.expr || '';
        nextState.cronTz = template.schedule.tz || '';
      }
    }
  }
...
const TaskForm: React.FC<TaskFormProps> = ({ mode, task, initialTemplate, onCancel, onSaved }) => {
  const [form, setForm] = useState<FormState>(() => createFormState(task, initialTemplate));
  ...
  useEffect(() => {
    setForm(createFormState(task, initialTemplate));
