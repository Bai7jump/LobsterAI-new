...
import TaskTemplateSelector, { getTemplateInput } from './TaskTemplateSelector';
...
const ScheduledTasksView: React.FC<ScheduledTasksViewProps> = ({
  ...
  const [pendingTemplate, setPendingTemplate] = useState<Partial<{
    name: string;
    description: string;
    schedule: any;
  ...
  const handleSelectTemplate = useCallback((templateId: string) => {
    const templateInput = getTemplateInput(templateId);
    setPendingTemplate(templateInput);
        ...
        <div className="flex items-center justify-between border-b dark:border-claude-darkBorder border-claude-border px-4 shrink-0">
          ...
          {activeTab === 'tasks' && (
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => dispatch(setViewMode('templates'))}
                className="px-3 py-1 text-sm font-medium border dark:border-claude-darkBorder border-claude-border dark:text-claude-darkText text-claude-text rounded-lg hover:bg-claude-surfaceHover dark:hover:bg-claude-darkSurfaceHover transition-colors"
              >
                {i18nService.t('scheduledTasksUseTemplate') || 'Use Template'}
              </button>
              <button
                type="button"
                onClick={() => {
